...
func VerifyUserStatus(){
	...
	infopath := config.INFOJSONFILE

	body, er := os.ReadFile(infopath)
	if(er != nil){
		funcs.CreateInfoJson()
	...
	if(Info.User.Active == false){
		fmt.Println("TelegramBot no configurado...")
	}
	...
	//fmt.Println("Bot de telegram no configurado")
	Active = false
...
func SendAlertTelegram(m string, t *time.Ticker){
	<- t.C //tiempo "espera"
	Send(m)
}
