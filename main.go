...
func main(){
	...
	data := make(chan string, 10) // prints 
	...
	for i:=0 ; i<len(filesToAnalize);i++{
		go dog.Analize(filesToAnalize[i], location, data)

	}
	
	for carpeta:=0 ; carpeta <len(InfoDirFiles.Dirs); carpeta ++{
		go dog.Analize(InfoDirFiles.Dirs[carpeta], location, data)
	}
	...
	go func(){
		...
		tickerTelegram := time.NewTicker(500 * time.Millisecond)//Tiempo razonable para enviar peticiones a telegra
		//tickerLog := time.NewTicker(50 * time.Millisecond)//tiempo que "espera" antes de escribir el archivo
		defer tickerTelegram.Stop()
		...
		for {
			...
			if(telegram.Active){
				telegram.SendAlertTelegram(mssg, tickerTelegram)
			}
